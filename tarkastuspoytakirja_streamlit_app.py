...
ALL_SECTIONS = list(SECTION_DEFINITIONS.keys())
DEFAULT_ENABLED = ["Yleistarkastus", "Keskijännitekojeisto", "Muuntaja", "Pääkeskus"]
...
def sync_sections_from_tarkastustyyppi() -> None:
    ...
    if tarkastuksen_tyyppi == "Muuntamotarkastus +ATEX":
        current.add("ATEX-tila")
    st.session_state.selected_sections = list(current)


def make_key(section: str, item_idx: int, suffix: str) -> str:
    return f"{section}_{item_idx}_{suffix}".replace(" ", "_")


def render_check_section(section_name: str, items: List[str]) -> None:
    st.subheader(section_name)
    st.caption("Merkitse havainto jokaisesta kohdasta ja lisää tarvittaessa kommentti.")
...
def gather_form_data() -> Dict:
    ...
    if tarkastuksen_tyyppi == "Muuntamotarkastus +ATEX" and "ATEX-tila" not in selected_sections:
    ...
    for section in selected_sections:
        report_data["tarkastusosiot"][section] = collect_section_data(section, SECTION_DEFINITIONS[section])
...
def load_json_to_state(data: Dict) -> None:
    ...
    st.session_state.selected_sections = sections if sections else DEFAULT_ENABLED.copy()
...
st.markdown(
    ...
    "Tarkastusosiot valitaan asetuksista kohdekohtaisesti."
)
...
with st.sidebar:
    ...
    st.multiselect(
        ...
        default=st.session_state.get("selected_sections", DEFAULT_ENABLED),
    )
...
selected_sections = st.session_state.get("selected_sections", [])
if st.session_state.get("tarkastuksen_tyyppi") == "Muuntamotarkastus +ATEX" and "ATEX-tila" not in selected_sections:
    selected_sections = selected_sections + ["ATEX-tila"]
...
if not selected_sections:
    ...
    for section in selected_sections:
        with st.expander(section, expanded=False):
            render_check_section(section, SECTION_DEFINITIONS[section])
